    ...
    def _amount_stp(self):
        for line in self:
            if line.purchase_price:
                ...
                #print('line.total_stp',line.total_stp)
    
    @api.onchange('product_uom', 'product_uom_qty')
    def product_uom_change(self):
        if not self.product_uom or not self.product_id:
            self.price_unit = 0.0
            #print("onchange prod uom ############")
            return
...
class SaleOrder(models.Model):
    ...
    def get_sale_year(self):
        #for order in self:
        if self.create_date:
            dt=datetime.strptime(self.create_date,"%Y-%m-%d %H:%M:%S").date()
            if dt.month>10:
    ...
    def write(self,values):
        for order in self:
            print("margin",order.margin)
            for id in order.order_line:
                print("order_line Margin",id.margin)
        rev_obj=self.env['revision.order']
    ...
    def approval_mail(self):
        ...
        mail.quit()
    #Validation for DR Done------___________#
    ...
    def action_confirm(self):
        ...
        #On create send mail to approval list
        #self.approval_mail()

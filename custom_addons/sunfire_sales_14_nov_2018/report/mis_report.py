...
class MisReport(models.Model):
	...
	pricelist_id = fields.Char('Currency')
	...
	def init(self):
		...
		self.env.cr.execute("""CREATE or REPLACE VIEW  %s as(
				...
				inner join line_of_business_info lob on lob.id=sl.line_of_business
...
class MisNewReport(models.Model):
	...
	cust_type=fields.Char(string='Customer Type')
	location=fields.Char(string='Billing Location')
	pricelist_id = fields.Char('Price List')
	...
	def init(self):
		tools.drop_view_if_exists(self.env.cr,self._table)
		self.env.cr.execute(""" CREATE or REPLACE VIEW %s as(
			select row_number() OVER () as id,confirmation_date,user_id,customer_name,opf_name,
			...
			,out_mnth,product_catagory,cust_type,location,pricelist_id,stata,confirmation_date
		) """%(self._table))